...
def buildOldPath(pi):
    ...
    if pl.cfeoKey > 0:
        ...
        if path is not None:
            oldPath = path.group(1) + "/" + path.group(1) + \
                "_" + path.group(2) + ".jp2"
        ...
        if path is not None:
            ...
            oldPath = path.group(1) + "/" + str(sl.witnessKey) + \
                "/" + path.group(2) + ".jp2"
    return oldPath
...
def verifyImageDimensions(pi, oldPath):
    ...
    try:
        ...
        if dimensions['width'] > 0:
            if pi.width != dimensions[
                    'width'] or pi.height != dimensions['height']:
...
def verifyImagesViaIIP():
    ...
    # 'http://ocve2-stg.cch.kcl.ac.uk/iip/iipsrv.fcgi?FIF=jp2/'
    fullurl = settings.IMAGE_SERVER_URL + '?FIF=jp2/'
