...
sqs = SearchQuerySet().filter(SQ(document='Source')
                              | SQ(document='Opus')).filter(cfeo=True).filter(live=True)\
    .order_by('orderno').facet('document')

urlpatterns = patterns('',
                       url(r'^browse/$', cfeoBrowse, name='cfeo_browse'),
                       url(r'^browse/image-preview/(?P<id>\d+)/$', cfeoImagePreview, name='cfeo_imagepreview'),
