...
def importAnnotations():
    ...
    cursor.execute(
        "select a.annotationKey,a.text,a.timestamp from annotation as a where a.ocve=-1")
    ...
    for row in cursor.fetchall():
        ...
        c2.execute(
            "select b.barNumber,b.barNumberExtra,ba.witnessKey from bar as b,barannotation as ba where ba.annotationKey=" +
            str(aKey) +
            " and b.barKey=ba.barKey")
        for barRow in c2.fetchall():
            regions = BarRegion.objects.filter(
                pageimage__page__sourcecomponent__source__sourcelegacy__witnessKey=int(
                    barRow[2]), bar__barnumber=int(
                    barRow[0]))
            for r in regions:
    ...
    for row in cursor.fetchall():
        ...
        if smatch:
            ...
            if highestBarNumber != endbar:
                ...
                log += '<li>pageimage id:' + str(p.id) + ', endbar:' + str(
                    p.endbar) + ' but end bar number:' + str(lowestBarNumber) + '</li>'
                p.save()
    log += '</ul></body></html>'
    return log


# A one-time function to correct problems in the merging of phase 1
# OCVE/CFEO databases
def uploadOCVEOpus28():
    ...
    stypes = [
        '', SourceType.objects.get(
            id=2), SourceType.objects.get(
            id=3), SourceType.objects.get(
                id=1)]
    ...
    for row in cursor.fetchall():
        try:
            si = SourceInformation.objects.get(
                source__sourcelegacy__witnessKey=int(row[5]))
            s = si.source
            ...
            if publisherAbbrev.__len__ > 0:
                try:
                    publisher = Publisher.objects.get(
                        publisherAbbrev=publisherAbbrev)
                except ObjectDoesNotExist:
                    # New publisher
                    publisher = Publisher(
                        publisherAbbrev=publisherAbbrev, publisher='Unknown')
                    publisher.save()
                except MultipleObjectsReturned:
                    publisher = Publisher.objects.filter(
                        publisherAbbrev=publisherAbbrev)[0]
            ...
            witCursor.execute(
                "SELECT Witness.witnessKey,Witness.pieceKey,Witness.sourceKey,Witness.CFEOeditionKey,"
                "Witness.witnessDescription,Witness.publisherAbbrev,Witness.full_text_c,Piece.opusPartNo FROM witness as Witness,piece as Piece where Witness.pieceKey=Piece.pieceKey and Witness.sourceKey=" +
                str(sourceKey) +
                " order by witnessKey")
            orderNo = 1
...
def initialValues():
    ...
    # SourceComponentType
    cursor.execute(
        "INSERT INTO ocve_sourcecomponenttype (type) VALUES('Unspecified')")
    cursor.execute(
        "INSERT INTO ocve_sourcecomponenttype (type) VALUES('Piano Score')")
    cursor.execute(
        "INSERT INTO ocve_sourcecomponenttype (type) VALUES('Single Instrument')")
    cursor.execute(
        "INSERT INTO ocve_sourcecomponenttype (type) VALUES('Multiple Instrument')")
    cursor.execute(
        "INSERT INTO ocve_sourcecomponenttype (type) VALUES('Orchestral')")
    cursor.execute(
        "INSERT INTO ocve_sourcecomponenttype (type) VALUES('Manuscript')")
    transaction.commit_unless_managed()
...
def importAuthorityLists():
    ...
    #cursor.execute("INSERT INTO ocve_country (country,countryabbrev) SELECT country,countryAbbrev FROM country")
    cursor.execute(
        "INSERT INTO ocve3.ocve_opus (opusno) SELECT opusNo FROM ocve3.opusal")
    ...
    # INSERT INTO ocve_bar (barnumber,barlabel) SELECT barNumber,barNumber
    # FROM bar;
    transaction.commit_unless_managed()
    importCountries()
    importCities()
...
def importCountries():
    ...
    cursor = connection.cursor()
    cursor.execute(
        "SELECT countryKey,country,countryAbbrev from Country order by countryKey")
    for row in cursor.fetchall():
...
def importBarRegions(pi, p, ocvePageKey):
    ...
    cursor.execute(
        "SELECT barKey,page_imageKey,x1,y1,x2,y2 FROM barregion where page_imageKey=" +
        str(ocvePageKey))
...
def importInstrument():
    cursor = connection.cursor()
    cursor.execute(
        "SELECT instrumentKey,instrumentName,instrumentAbbrev FROM instrumental ")
    for row in cursor.fetchall():
        Instrument(
            id=int(
                row[0]), instrument=str(
                row[1]), instrumentabbrev=str(
                row[2])).save()
...
def lookupGenre(genreKey, CFEO):
    ...
    if CFEO == 1:
        cursor.execute(
            "SELECT genreKey,genreName,altName from algenre where genreKey=" +
            str(genreKey))
        row = cursor.fetchone()
    else:
        cursor.execute(
            "SELECT genreKey,genreName,pluralName from genreal where genreKey=" +
            str(genreKey))
        row = cursor.fetchone()
    if row is not None:
        try:
...
def importArchives():
    ...
    global archives
    cursor.execute(
        "SELECT archiveKey,cityKey,archiveName,archiveSiglum from o_archive ")
    for row in cursor.fetchall():
...
def parseSourceDesc(s, sourceDesc):
    ...
    codeMatch = re.search(
        "<label>Source code</label>\s*<value>(.*?)</value>",
        sourceDesc,
        re.IGNORECASE | re.MULTILINE)
    infoMatch = re.search(
        "<label>Additional information</label>\s*<value>(.*?)</value>",
        sourceDesc,
        re.IGNORECASE | re.MULTILINE)
    featuresMatch = re.search(
        "<heading>Key features:</heading>\s*<para>(.*?)</para>",
        sourceDesc,
        re.IGNORECASE | re.MULTILINE)
    contentsSummaryMatch = re.search(
        "<label>Contents</label>\s*<value><heading>(.*?)</heading>",
        sourceDesc,
        re.IGNORECASE | re.MULTILINE)
    publicationTitleMatch = re.search(
        "<label>Publication title</label>\s*<value>(.*?)</value>",
        sourceDesc,
        re.IGNORECASE | re.MULTILINE)
    placePublicationMatch = re.search(
        "<label>Place of publication</label>\s*<value>(.*?)</value>",
        sourceDesc,
        re.IGNORECASE | re.MULTILINE)
    publicationDateMatch = re.search(
        "<label>Publication date</label>\s*<value>(.*?)</value>",
        sourceDesc,
        re.IGNORECASE | re.MULTILINE)
    sourceCode = ''
    ...
    dedicateeMatch = re.search(
        "<label>Dedicatee</label>\s*<value>(.*?)</value>",
        sourceDesc,
        re.IGNORECASE | re.MULTILINE)
    dedicatee = None
...
def importWitness():
    cursor = connection.cursor()
    # todo:
    # ChristopheTextWitness,Witness.notes,Witness.designation,Witness.designationSuperscript,
    # removed for now due to unicode errors
    cursor.execute(
    ...
    for row in cursor.fetchall():
        ...
        wc = pieces[int(row[1])]
        SourceComponent_WorkComponent(
            sourcecomponent=newSComp,
            workcomponent=wc).save()
...
def importPageImage(sc, witnessKey):
    ...
    #
    cursor.execute(
        "SELECT p.pageID,p.orderNo,p.startBar,p.endBar,p.sourceWidth,p.sourceHeight,p.page_imageKey,p.filename,p.storageStructure FROM page_image as p,witnesspage_intersection as wp WHERE wp.pageImageKey=p.page_imageKey and wp.witnessKey=" +
        str(witnessKey) +
        " order by p.orderNo")
    ...
    for row in cursor.fetchall():
        ...
        p.save()
        pi = PageImage(
            surrogate=0, versionnumber=1, startbar=str(
                row[2]), endbar=str(
                row[3]), permission=1, permissionnote='', width=int(
                row[4]), height=int(
                    row[5]), page=p)
        pi.save()
        # Page Legacy is for tying images to pages, temporary until full
        # renaming complete
        filename = ''
        ...
        dimensions = verifyImageDimensions(
            pi, prefix + storageStructure + '.jp2')
        pl = PageLegacy(
            pageimage=pi,
            cfeoKey=0,
            filename=filename,
...
def getPublisher(key):
    cursor = connection.cursor()
    cursor.execute(
        "SELECT publisherKey,publisherAbbrev,publisherName from publisheral where publisherKey=" +
        str(key))
    for row in cursor.fetchall():
        abbrev = row[1]
        try:
...
def addWorkCollection():
    WorkCollection(
        id=1,
        label="Unspecified",
        overridelabel="Unspecified").save()
...
def verifySourceComponent(comp, volumeKey, workcomp):
    ...
    cursor = connection.cursor()
    cursor.execute(
        "SELECT textID from volume where volumeKey=" +
        str(volumeKey))
    # Get volume
    for row in cursor.fetchall():
        ...
        if row[0] is not None and label.__len__() > 0:
            ...
            if m is not None:
                ...
                try:
                    curIns = Instrument.objects.get(
                        sourcecomponent_instrument__sourcecomponent=comp)
                except ObjectDoesNotExist:
                    curIns = None
                if curIns is None or instrument != curIns or comp.instrumentnumber != num:
                    try:
                        sc = SourceComponent.objects.get(
                            source=comp.source,
                        ...
                        sc.save()
                        SourceComponent_WorkComponent(
                            sourcecomponent=sc, workcomponent=workcomp).save()
...
def importCFEOPages(comp, compKey, editionKey):
    ...
    for row in cursor.fetchall():
        ...
        if re.search("score", notes, re.IGNORECASE | re.MULTILINE) is not None:
            ...
            pt = ptt
            # Check if instruments are involved, and more sourcecomponents need
            # to be created
        workcomps = WorkComponent.objects.filter(
            sourcecomponent_workcomponent__sourcecomponent=comp)
...
def importEditions():
    ...
    for row in cursor.fetchall():
        ...
        try:
            ...
            scType = SourceComponentType.objects.get(type="Piano Score")
            sc = SourceComponent.objects.get(
                sourcecomponenttype=scType, source=s)
        ...
        if si is not None:
            ...
            si = SourceInformation(
                ...
                dedicatee=d,
                platenumber=plate,
                locationsimilarcopies=locationsimilarcopies,
                reprints=reprints,
                seriestitle=seriestitle,
                title=cfeolabel,
                copyright=copyright,
                contents=contents,
                leaves=contentssummary,
                datepublication=datepublication,
                shelfmark=shelfmark,
                notes=notes,
                additionalInformation='')
        ...
        SourceInformation_PrintingMethod(
            sourceinformation=si, printingmethod=method).save()
        # Work
        w = None
        if sc is not None:
            # Work already imported, add CFEO orderno
            w = Work.objects.get(
                workcomponent__sourcecomponent_workcomponent__sourcecomponent=sc)
    ...
    # Remove any 'orphan' source components, usually caused by multiple
    # instruments
    ...
    for c in comps:
        ...
        if pages is None or pages.__len__() == 0:
            ...
            links = SourceComponent_WorkComponent.objects.filter(
                sourcecomponent=c)
            for l in links:
                l.delete()
            c.delete()
...
def getWorkOrder(workKey):
    cursor = connection.cursor()
    cursor.execute(
        "SELECT Work.orderNo from Work where Work.workKey=" +
        str(workKey))
    orderNo = 99
    for row in cursor.fetchall():
        orderNo = row[0]
...
def importCFEOWork(workKey):
    ...
    for row in cursor.fetchall():
        ...
        try:
            ...
            # Genres
            cursor.execute(
                "SELECT workKey,genreKey from genrework where workKey=" +
                str(workKey) +
                "")
            for row in cursor.fetchall():
...
def importCFEOWorkComponent(source, work, editionKey, inOCVE):
    ...
    cursor.execute(
        ...
        " FROM ocve3.workcomponent as wc,PageBars as pb,Page as p,ocve3.work as w where p.editionKey=" +
        str(editionKey) +
        " and w.workKey=wc.workKey " +
    ...
    for row in cursor.fetchall():
        ...
        if music == 1:
            ...
            if inOCVE == 0:
                ...
                newWComp = None
                try:
                    newWComp = WorkComponent.objects.get(
                        label=label, work=work)
                except ObjectDoesNotExist:
                    # Create Workcomponent
                    newWComp = WorkComponent(
                        music=music, label=label, orderno=int(
                            row[6]), work=w, keymode=km, keypitch=kp, opus=o)
                    newWComp.save()
                ...
                SourceComponent_WorkComponent(
                    sourcecomponent=newSComp, workcomponent=newWComp).save()
...
def importPosthumous():
    ...
    for row in cursor.fetchall():
        ...
        if int(row[2]) > 0:
            ...
            # Create Workcomponent
            newWComp = WorkComponent(
                music=1, label=label, orderno=int(
                    row[5]), work=w, keymode=km, keypitch=kp, opus=o)
            newWComp.save()
    ...
    for row in cursor.fetchall():
        ...
        if info.count() > 0:
            ...
            if len(si.displayedcopy) < 1:
                if witnessKey > 0:
                    displayMatch = re.search(
                        "<label>Displayed copy</label>\s*<value>(.*?)</value>",
                        sourceDesc,
                        re.IGNORECASE | re.MULTILINE)
                    ...
                    cursor2.execute(
                        "SELECT shelfmark FROM ocve2real.edition where editionKey=" +
                        str(cfeoKey))
            ...
            if si.placepublication_id < 2:
                if cfeoKey > 0:
                    cursor2 = connection.cursor()
                    cursor2.execute(
                        "SELECT placepublication FROM ocve2real.edition where editionKey=" +
                        str(cfeoKey))
                    for row2 in cursor2.fetchall():
                        city = str(row2[0])
                    try:
...
def correctSourceInformation(request):
    cursor = connection.cursor()
    infos = SourceInformation.objects.filter(
        source__sourcelegacy__witnessKey__gt=0)
    # todo:
    # ChristopheTextWitness,Witness.notes,Witness.designation,Witness.designationSuperscript,
    # removed for now due to unicode errors
    ...
    for si in infos:
        witnessKey = SourceLegacy.objects.get(
            source_id=si.source_id).witnessKey
        s = si.source
        cursor.execute(
            "SELECT Witness.witnessKey,Witness.pieceKey,Witness.sourceKey,Witness.CFEOeditionKey,Witness.witnessDescription,Witness.publisherAbbrev,Witness.full_text_c FROM witness as Witness where Witness.witnessKey=" +
            str(witnessKey))
        for row in cursor.fetchall():
            # Create new source
            st = getSourceType(row[2])
