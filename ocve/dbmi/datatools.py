...
def correctSourceInformation(request):
    ...
    for si in infos:
        ...
        for row in cursor.fetchall():
            ...
            publisherAbbrev = row[5]
            sourceDesc = row[6]
            # if len(si.platenumber) < 1:
            #     plateMatch = re.search("<label>Plate number</label>\s*<value>(.*?)</value>", sourceDesc, re.IGNORECASE | re.MULTILINE)
            #     if plateMatch is not None:
            #         plate=plateMatch.group(1)
            #         if len(plate) > 0:
            #             si.platenumber=plate
            #             si.save()
            #
            # # </field> <field> <label>Displayed copy</label> <value>US-Cu: M25.C54 B123 c.1</value> </field>
            if len(si.displayedcopy) < 1:
                displayMatch = re.search("<label>Displayed copy</label>\s*<value>(.*?)</value>", sourceDesc,
                                         re.IGNORECASE | re.MULTILINE)
                if displayMatch is not None:
                    ...
                    if len(display) > 0:
                        ...
                        si.save()
                        #
                        # #if si.archive == emptyArchive:
                        # if si.placepublication is None or si.placepublication == emptyCity:
                        #     cityMatch=re.search("<label>Place of publication</label>\s*<value>(.*?)</value>", sourceDesc, re.IGNORECASE | re.MULTILINE)
                        #     if cityMatch is not None:
                        ...
                        #             si.save()
                        #
                        # if si.publisher == emptyPublisher:
                        #     publisherMatch = re.search("<label>Publisher</label>\s*<value>(.*?)</value>", sourceDesc, re.IGNORECASE | re.MULTILINE)
                        #     if publisherMatch is not None:
                        #         publisher=publisherMatch.group(1)
                        #         if len(publisher) > 0:
                        #             try:
                        #                 p=Publisher.objects.get(publisher=publisher)
                        #             except ObjectDoesNotExist:
                        ...
                        #             si.save()
                        #
                        # if len(si.contents) < 1:
                        #     log+="<tr><td>"+str(witnessKey)+"</td><td>"+sourceDesc+"</td></tr>"
                        #     #<label>p. 2</label><barRange>bs 1&#150;26</barRange>
                        #     contentsMatch = re.search("<pageList>(.*?)</pageList>", sourceDesc,re.IGNORECASE | re.MULTILINE)
                        #     if contentsMatch is not None:
                        #         contentsXML=contentsMatch.group(1)
                        #         contents='<ul>'+contentsXML.replace('page>','li>').replace('label>','span>').replace('barRange>','span>')+'</ul>'
                        #         si.contents=contents
                        #         #si.save()
                        #
                        # if si.dedicatee == emptyDedicatee:
                        #     dedicateeMatch = re.search("<label>Dedicatee</label>\s*<value>(.*?)</value>", sourceDesc, re.IGNORECASE | re.MULTILINE)
                        #     if dedicateeMatch is not None:
                        #         dedicatee=dedicateeMatch.group(1)
                        #         if len(dedicatee) > 0:
                        #             try:
                        #                 d=Dedicatee.objects.get(dedicatee=dedicatee)
                        #             except ObjectDoesNotExist:
                        #                 d=Dedicatee(dedicatee=dedicatee)
                        #                 d.save()
                        #             si.dedicatee=d
                        #             si.save()
