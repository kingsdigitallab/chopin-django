...
def fix(request):
    ...
    cphtml = ''
    for n in np:
        cp = 'cp "' + settings.IMAGEFOLDER + 'temp/' + n.filename + '" "' + \
            settings.IMAGEFOLDER + 'jp2/newjp2/' + str(n.id) + '.jp2"'
        cphtml = cphtml + cp + '\n'
        os.system(cp)
...
def convertFolder(request, folderName):
    ...
    files = os.listdir(
        os.path.join(
            settings.CONVERTED_UPLOAD_PATH,
            folderName))
    for f in files:
        fullurl = settings.IMAGE_SERVER_URL + '?FIF=' + \
            settings.CONVERTED_UPLOAD_PATH + '/' + f
...
# Convert a single image to a page in an existing source, pass to the
# editor for identification
@csrf_exempt
    ...
    if s is not None:
        ...
        if folderName is not None:
            for image in imageNames:
                image = folderName.encode(
                    'UTF-8') + '/' + image.encode('UTF-8')
