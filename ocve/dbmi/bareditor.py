...
def editBars(request, id):
    ...
    try:
        ...
        # Redirect to the crop correct view if uncorrected before displaying
        # editor
        ...
        if pl.cfeoKey > 0:
            ...
            oldPath = "jp2/ocvejp2-proc/" + \
                path.group(1) + "/" + str(sl.witnessKey) + "/" + path.group(2) + ".jp2"
            verifyImageDimensions(pi, oldPath)
            zoomifyURL = zoomifyURL + oldPath + "/"
            ...
            if bars.count() > 0:
                ...
                for b in bars:
                    if len(label) > 0:
                        label += ', '
                    label += b.barlabel
                regionString = regionString + '<span data-regionid=\"' + \
                    str(r.id) + '\">' + label + '</span>, '
        ...
        try:
            ...
            prev = PageImage.objects.get(
                page__orderno=before,
                page__sourcecomponent=p.sourcecomponent)
        ...
        if r.anomaly == 0:
            ...
            try:
                bars = Bar.objects.filter(
                    barnumber=barNum, barlabel__iexact=str(barLabel))
                if bars.count() > 0:
                    b = bars[0]
            except ObjectDoesNotExist:
            ...
            if barNum == int(
                re.sub(
                    '[a-z|A-Z]*',
                    '',
                    pi.endbar)) and re.match(
                '[a-z|A-Z]+',
                    str(barNum)) is not None:
                barLabel = pi.endbar
            else:
                barLabel = barNum
        else:
        ...
        for number in numbers.split(","):
            ...
            if bars.count() > 0:
                ...
                if m is not None:
                    ...
                    barlink = Bar_BarRegion(bar=b, barregion=br)
                    barlink.save()
                    # Force anomaly if multiple numbers assigned, in case user
                    # forgets
...
def updateBarRegion(request):
    ...
    try:
        ...
        br.delete()
        return HttpResponse(
            "{\"message\":\"Region deleted\",\"id\":" + str(deleteID) + "}",
    ...
    try:
        ...
        return HttpResponse("{\"message\":\"Region added\",\"id\":" +
                            str(br.id) + "}", content_type="application/json")
    except MultiValueDictKeyError:
        pass
        # Update all regions
