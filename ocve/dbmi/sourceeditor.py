...
#Reorder the pages in a source based on the label.  Very crude but good to correct earlier errors
def defaultpageorder(request,id):
    source=Source.objects.get(id=id)
    pageimages = PageImage.objects.filter(page__sourcecomponent__source=source).order_by('sourcecomponent_orderno','page__orderno')
...
def createnonmusiccomponent(request,label,orderno,source):
    ...
    sc=SourceComponent(source=source,label=label,orderno=orderno,sourcecomponenttype=sctype)
    sc.save()
    SourceComponent_Instrument(instrument=noinstrument,sourcecomponent=sc).save()
...
def savesourcecomponent(request,id):
    ...
    try:
        ...
        pass
    return HttpResponseRedirect("/ocve/sourceeditor/"+str(sc.source.id)+"/")
...
def clonePageImage(pi,newPageImages,newSourceComponents):
    ...
    pi.page=p
    pi.save()
    return [pi,newPageImages,newSourceComponents]
    ...
    if regions.count() > 0:
        for r in regions:
            ...
            if newPageImages[str(pi.id)] is None:
                [pi,newPageImages,newSourceComponents]=clonePageImage(pi,newPageImages,newSourceComponents)
            else:
                #Already cloned
