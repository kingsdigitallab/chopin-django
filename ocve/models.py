...
SourceComponent._meta.ordering = ["orderno"]
...
#        jp2 = 'jp2/cfeojp2-proc/' + oldPath
#    elif pl.ocveKey>0:
#        path = re.search("(\d+)\/.*?\/(.*)", pl.storageStructure)
#        sl = SourceLegacy.objects.get(source=self.page.sourcecomponent.source)
#        oldPath = path.group(1) + "/" + str(sl.witnessKey) + "/" + path.group(2) + ".jp2"
#        jp2 = 'jp2/ocvejp2-proc/' + oldPath
#    else:
#        jp2 = pl.jp2
...
def getZoomifyPath(self):
    ...
    #            path = re.search("(.*?)_(.*)", pl.filename)
    #            oldPath = path.group(1) + "/" + path.group(1) + "_" + path.group(2) + ".jp2"
    #            fullurl = fullurl + jp2+ '&obj=IIP,1.0&obj=Max-size&obj=Tile-size&obj=Resolution-number'
    #        elif pl.ocveKey>0:
    #            path = re.search("(\d+)\/.*?\/(.*)", pl.storageStructure)
