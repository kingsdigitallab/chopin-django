...
class Work(models.Model):
    ...
    def getSources(self):
        sources = Source.objects.filter(
            sourcecomponent__sourcecomponent_workcomponent__workcomponent__work=self).distinct()
        return sources
...
class Source(models.Model):
    ...
    def getWork(self):
        works = Work.objects.filter(
            workcomponent__sourcecomponent_workcomponent__sourcecomponent__source=self).distinct()
    ...
    def getPrimaryPageImages(self):
        pi = PageImage.objects.filter(
            page__sourcecomponent__source=self,
    ...
    def getFirstBarRegion(self):
        try:
            return BarRegion.objects.filter(
                pageimage__page__sourcecomponent__source=self).order_by(
                'pageimage__page', 'bar')[0]
        except ObjectDoesNotExist:
            return None
        except IndexError:
            return None
...
class Instrument(models.Model):
    ...
    instrumentabbrev = models.CharField(
...
class SourceComponent(models.Model):
    ...
    instrumentkey = models.CharField(
        max_length=128, null=False, default="", blank=True, )
    overridelabel = models.TextField(null=False, default="", blank=True, )
    sourcecomponenttitle = models.CharField(
        max_length=255,
        null=False,
        default="",
        blank=True,
...
class SourceInformation(models.Model):
    ...
    locationsimilarcopies = models.TextField(
    ...
    contentssummary = models.CharField(
    ...
    printingmethod = models.ManyToManyField(
        'PrintingMethod',
        through='SourceInformation_PrintingMethod',
        default=1)
...
class PageImage(models.Model):
    ...
    permissionnote = models.CharField(
        max_length=255, null=False, default="", blank=True, )
    height = models.IntegerField(null=True, blank=True, )
    width = models.IntegerField(null=True, blank=True, )
    startbar = models.CharField(
...
class Page(models.Model):
    ...
    def getWorkComponent(self):
        wc = WorkComponent.objects.filter(
            sourcecomponent_workcomponent__sourcecomponent__page=self)
        if wc.count() > 0:
            return wc[0]
        return None
...
class Country(models.Model):
    ...
    countryabbrev = models.CharField(
        max_length=128, null=False, default="", blank=True, )
...
def getWorkComponentLabel(self):
    label = ''
    try:
        wc = WorkComponent.objects.get(
            sourcecomponent_workcomponent__sourcecomponent=self)
        label = wc.label
    except ObjectDoesNotExist:
        pass
    except MultipleObjectsReturned:
...
class NewPageImage(models.Model):
    ...
    # >0 if it is saved in pages/page images
    linked = models.IntegerField(null=True, blank=True, default=0)
