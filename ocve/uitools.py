...
class SourceComponentItem:
    def __init__(self, sourcecomponent):
        ...
        self.instrument_id = sourcecomponent.instrumentkey
    ...
    def toJson(self):
        ...
        scjson += "'instrument_id':" + json.dumps(self.instrument_id)
        scjson += "}"
        return scjson
...
class SourceSearchItem:
    ...
    def toJson(self):
        sourcejson =  "{'id': " + str(self.id) + ", 'label': " + json.dumps(self.label) + ", "
        sourcejson +=  "'Genre': ["
        x = 1
        ...
        if self.mode == 'OCVE':
            ...
            pages = PageImage.objects.filter(page__sourcecomponent__source_id=self.id,page__pagetype=musicpage).order_by("page")
        else:
...
class PageSearchItem:
    def __init__(self, source,work, pageimage,dedicatee,publisher,years):
        ...
        self.genres = genres
        instruments = []
        for i in pageimage.getInstruments():
...
def serializeSourceJson(sourcecomponents,filename,mode):
    ...
    destination = open(settings.PROJECT_ROOT+"/"+settings.STATIC_ROOT+settings.SOURCEJSONPATH+'/'+filename+'.js', 'w')
    ...
    destination.close()
    #Gzip results
    f_in = open(settings.PROJECT_ROOT+"/"+settings.STATIC_ROOT+settings.SOURCEJSONPATH+'/'+filename+'.js', 'rb')
    f_out = gzip.open(settings.PROJECT_ROOT+"/"+settings.STATIC_ROOT+settings.SOURCEJSONPATH+'/'+filename+'.js.gz', 'wb')
...
def serializeAcCodeConnector():
    ...
    destination = open(settings.PROJECT_ROOT+"/"+settings.STATIC_ROOT+settings.SOURCEJSONPATH+ '/accodeJSON.js', 'wb')
    destination.write('var sources = [')
    first = 0
    for s in sources:
